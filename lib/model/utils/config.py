...
def cfg_from_file(file_path):
    with open(file_path,'r') as f:
        cfg_load=edict(yaml.load(f))
    merge_a_to_b(cfg_load,cfg)
...
def merge_a_to_b(a,b):
    ...
    for k,v in a.items():
        ...
        # 判断a中的数值类型与b中是否对应
        old_type=type(b[k])
        if type(v) is not old_type:
            if isinstance(old_type,np.ndarray):
        ...
        if type(v) is edict:
            try:
                merge_a_to_b(a[k],b[k]) # 递归处理edict类型的值，类似于处理多层嵌套edict
            except:
                raise
        else:
            b[k]=v
...
def cfg_from_list(cfg_list):
    assert len(cfg_list)%2==0 # 判断cfg_list是否是偶数长度（key与val必须一一对应）
    for k,v in zip(cfg_list[0::2],cfg_list[1::2]):
        k_=k.split('.') # 可能会是多层关键字
        d=__C
        # 判断cfg_list中的key是否存在于_C中
        for subkey in k_[:-1]:
            assert subkey in d
            d=d[subkey]
        subkey=k_[-1]
        ...
        # 将字符串格式的值进行转换，若原本就应该是string类型，则会执行except语句
        try:
            val=literal_eval(v)
        except:
            val=v
        assert type(val)==type(d[subkey]),'type {} does not match original type {}'.format(
                                            type(val), type(d[subkey]))
        d[subkey]=val